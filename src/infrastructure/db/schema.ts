...
export const blockIdTable = pgTable(
  'blockId', 
  {
    ...
    rootHash: varchar({ length: 64 }).unique().notNull(),
    fileHash: varchar({ length: 64 }).unique().notNull(),
  },
  (table) => [
...
export const depositTable = pgTable(
  'deposits_tx', 
  {
    ...
    shardRootHash: varchar({ length: 64 }).notNull(),
    shardFileHash: varchar({ length: 64 }).notNull(),
